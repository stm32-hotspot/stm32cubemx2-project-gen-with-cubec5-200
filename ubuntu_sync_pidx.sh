...
parse_installed_packs() {
    get_installed_packs | awk '
    {
        ...
        # Find where the version starts: first dot followed by a digit
        if (!match(line, /\.[0-9]/)) {
            next
        }

        # Position of the dot right before the version
        index_version = RSTART

        # Extract vendor + name prefix
        ...
        # Rebuild full pack name in case it contains dots
        name = prefix_parts[2]
        for (i = 3; i <= n; i++) {
            name = name "." prefix_parts[i]
        }

        # Extract full version starting after the dot
